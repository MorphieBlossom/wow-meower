...
function ER:OnChatMsgEmote(text, sender, language, channelString, target, flags, unknown, channelNumber, channelName,
                           unknown2, counter, guid)
...
function ER:ProcessReply(triggerToken, sender)
  if not addon.DB.Settings.replies then return end

  for _, reply in ipairs(addon.DB.Settings.replies) do
    if reply.trigger == triggerToken then
      ...
      elseif reply.replyType == "CUSTOM" then
        ...
        C_ChatInfo.SendChatMessage(reply.replyValue, "EMOTE")
      end
