...
function Settings:CreateConfig()
  ...
  f.TitleText:SetText(addon.NAME)
  ...
  enableCb:SetChecked(addon.DB.Settings.enabled)
  enableCb:SetScript("OnClick", function(self) addon.DB.Settings.enabled = self:GetChecked() end)
  ...
  welcomeCb:SetChecked(addon.DB.Settings.showWelcome)
  welcomeCb:SetScript("OnClick", function(self) addon.DB.Settings.showWelcome = self:GetChecked() end)
  ...
  local function RefreshReplyList()
    ...
    for i, reply in ipairs(addon.DB.Settings.replies) do
      ...
      delBtn:SetScript("OnClick", function()
        table.remove(addon.DB.Settings.replies, i)
        RefreshReplyList()
      end)
  ...
  addBtn:SetScript("OnClick", function()
    ...
    if trig and rVal and trig ~= "" and rVal ~= "" then
      tinsert(addon.DB.Settings.replies, {
        trigger = trig,
        replyType = rType,
        replyValue = rVal
      })
...
function Settings:ToggleConfig()
